...
游戏的命中率分为 **物理命中率** 和 **魔法命中率** 两套独立系统。

#### 物理命中率

```
最终命中率 = 武器固有命中率 × (1 + PhysicalHitRatio%) + Hitfix
```

| 字段 | 来源 | 说明 |
|------|------|------|
| 武器固有命中率 | 武器物品数据 | 每把武器自带（90%～99%），在物品描述中显示为「命中:XX%」 |
| PhysicalHitRatio | 装备/技能加成 | 存档字段 `FightAttr.PhysicalHitRatio`、`PermanentFightAttr.PhysicalHitRatio` |
| Hitfix | 状态/修正 | 存档字段 `entity.Hitfix`，状态异常或特殊效果的命中修正 |

**提高物理命中率的装备：**

| 装备 | 类型 | 效果 |
|------|------|------|
| 冥王之铠 (ID 113) | 铠甲 | 武器命中率+10% |
| 邪眼假面 (ID 136) | 头盔 | 武器命中率+10% |
| 影杀之靴 (ID 151) | 鞋子 | 武器命中率+10% |
| 猫眼戒指 (ID 161) | 饰品 | 武器命中率+5% |

#### 魔法命中率

```
最终命中率 = 基础命中率 + MagicHitRatio%
```

| 字段 | 来源 | 说明 |
|------|------|------|
| MagicHitRatio | 装备/技能加成 | 存档字段 `FightAttr.MagicHitRatio`、`PermanentFightAttr.MagicHitRatio` |

**提高魔法命中率的装备：**

| 装备 | 类型 | 效果 |
|------|------|------|
| 学者眼镜 (ID 173) | 饰品 | 魔法命中率+10% |
| 魔操玉 (ID 184) | 饰品 | 魔法命中+10% |

#### 实际命中概率

最终命中概率还受双方属性影响：

- 攻击方的 **Dex（敏捷）** 越高，物理命中率越高
- 攻击方的 **Mind（智力）** 越高，魔法命中率越高
- 防御方的 **DodgeRatio（闪避率）** 越高，被命中的概率越低
- **CriticalRatio（暴击率）** 独立于命中率，暴击必定命中
